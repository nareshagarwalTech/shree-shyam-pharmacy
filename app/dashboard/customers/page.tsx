...
import { Search, Pill, ChevronDown, ChevronUp, Trash2 } from 'lucide-react';
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2 rounded-lg border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 w-64"
            />
          </div>
        </div>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 bg-emerald-100 rounded-full flex items-center justify-center">
                      <span className="text-emerald-700 font-semibold">{customer.name.charAt(0).toUpperCase()}</span>
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-4">
                    <span className="text-sm text-gray-500">{customer.medications.length} medication(s)</span>
                    {expandedCustomer === customer.id ? <ChevronUp className="w-5 h-5 text-gray-400" /> : <ChevronDown className="w-5 h-5 text-gray-400" />}
                  </div>
                </div>

                {expandedCustomer === customer.id && (
                  <div className="border-t border-gray-100 p-4 bg-gray-50">
                    {customer.address && <p className="text-sm text-gray-600 mb-4"><span className="font-medium">Address:</span> {customer.address}</p>}
                      ...
                      {customer.medications.length === 0 ? (
                        ...
                        <div className="grid gap-3">
                          {customer.medications.map((med) => (
                            <div key={med.id} className="flex items-center gap-3 p-3 bg-white rounded-lg border border-gray-200">
                              <Pill className="w-5 h-5 text-emerald-500" />
                              <div className="flex-1">
                                <p className="font-medium text-gray-900">{med.name}</p>
                                <p className="text-sm text-gray-500">{med.quantity} units • {med.daily_dosage}/day • Refill: {formatDate(med.refill_date)}</p>
                              </div>
                            </div>
                          ))}
                        </div>
